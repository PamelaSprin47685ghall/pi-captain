...
import type { Runnable, Agent, AgentName, PipelineState, StepResult, CaptainDetails } from "./types.js";
import { executeRunnable, type ExecutorContext } from "./executor.js";
...
export default function (pi: ExtensionAPI) {
  ...
  function parseMdAgent(filePath: string): Agent | null {
    ...
    const getName = (s: string) => s.split("\n").find(l => l.startsWith("name:"))?.slice(5).trim();
    const getDesc = (s: string) => s.split("\n").find(l => l.startsWith("description:"))?.slice(12).trim();
    const getTools = (s: string) => s.split("\n").find(l => l.startsWith("tools:"))?.slice(6).trim();
    ...
    return {
      ...
      tools: getTools(fm)?.split(",").map(t => t.trim()) ?? [],
  ...
  const reconstruct = (ctx: ExtensionContext) => {
    ...
    for (const entry of ctx.sessionManager.getBranch()) {
      ...
      // Each captain tool stores a CaptainDetails snapshot
      if (
        msg.toolName === "captain_define" ||
        msg.toolName === "captain_load" ||
        msg.toolName === "captain_run" ||
        msg.toolName === "captain_list" ||
        msg.toolName === "captain_status" ||
        msg.toolName === "captain_agent" ||
        msg.toolName === "captain_generate"
      ) {
        const d = msg.details as CaptainDetails | undefined;
        if (d) {
    ...
    // Always re-load .md agents as baseline (runtime agents from session take precedence)
    loadMdAgents();
  };
  ...
  function snapshot(lastRun?: PipelineState): CaptainDetails {
    return {
      pipelines: { ...pipelines },
      agents: { ...agents },
      lastRun: lastRun ? { name: lastRun.name, state: { ...lastRun } } : undefined,
    };
  }
  ...
  pi.registerTool({
    name: "captain_agent",
    label: "Captain Agent",
    description: "Define a reusable agent config with name, description, tools, model, and temperature. Agents are referenced by name in pipeline steps.",
    parameters: Type.Object({
      name: Type.String({ description: "Unique agent name" }),
      description: Type.String({ description: "What this agent does" }),
      tools: Type.String({ description: "Comma-separated tool names (e.g. 'read,bash,edit')" }),
      model: Type.Optional(Type.String({ description: "Model identifier (e.g. 'sonnet', 'flash')" })),
      temperature: Type.Optional(Type.Number({ description: "Sampling temperature (0-1)" })),
    }),
    ...
    async execute(_id, params) {
      ...
      return {
        content: [{ type: "text", text: `Agent "${params.name}" defined: ${params.description} (tools: ${agent.tools.join(", ")})` }],
        details: snapshot(),
      };
    },

    renderCall: (args, theme) =>
      new Text(theme.fg("toolTitle", theme.bold("captain_agent ")) + theme.fg("accent", args.name), 0, 0),
    renderResult: (result, _opts, theme) => {
      ...
      const mdCount = Object.values(d.agents).filter(a => a.source === "md").length;
      const rtCount = total - mdCount;
      return new Text(theme.fg("success", `✓ ${total} agent(s) (${mdCount} md, ${rtCount} runtime)`), 0, 0);
    },
  });
  ...
  function discoverPresets(cwd: string): { name: string; source: "builtin" | "project" }[] {
    ...
    if (existsSync(projectDir)) {
      for (const f of readdirSync(projectDir).filter(f => f.endsWith(".json"))) {
  ...
  function loadBuiltinPreset(name: string): { name: string; agentCount: number; spec: Runnable } {
  ...
  function loadPipelineFile(filePath: string): { name: string; agentCount: number; spec: Runnable } {
    ...
    if (!data.pipeline?.kind) {
      throw new Error("Invalid pipeline file: missing 'pipeline' with 'kind' field");
    }
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      action: StringEnum(["load", "list"] as const),
      name: Type.Optional(Type.String({ description: "Preset name or file path (required for 'load')" })),
    }),
    ...
    async execute(_id, params, _signal, _onUpdate, ctx) {
      ...
      if (params.action === "list") {
        ...
        if (presets.length === 0) {
          return {
            content: [{ type: "text", text: "No presets found. Add .ts modules to pipelines/ or .json files to .pi/pipelines/" }],
            details: snapshot(),
          };
        }
        ...
        return {
          content: [{ type: "text", text: `Available pipeline presets:\n${lines.join("\n")}` }],
          details: snapshot(),
        };
      }
      ...
      if (!params.name) {
        return {
          content: [{ type: "text", text: "Error: 'name' is required for load action. Use action 'list' to see available presets." }],
          isError: true,
        };
      }
      ...
      try {
        ...
        if (builtinPresetMap[params.name!]) {
          const result = loadBuiltinPreset(params.name!);
          const summary = describeRunnable(result.spec, 0);
          return {
            content: [{
              type: "text",
              text: `Loaded builtin pipeline "${result.name}" (${result.agentCount} agents)\n\n${summary}`,
            }],
            details: snapshot(),
          };
        }
        ...
        const projectFile = join(cwd, ".pi", "pipelines", `${params.name}.json`);
        if (existsSync(projectFile)) {
          const result = loadPipelineFile(projectFile);
          const summary = describeRunnable(result.spec, 0);
          return {
            content: [{
              type: "text",
              text: `Loaded project pipeline "${result.name}" (${result.agentCount} agents)\n\n${summary}`,
            }],
            details: snapshot(),
          };
        }
        ...
        const candidate = resolve(cwd, params.name!);
        const filePath = existsSync(candidate) ? candidate
          : existsSync(params.name!) ? resolve(params.name!)
          : undefined;
        ...
        if (filePath) {
          ...
          return {
            content: [{
              type: "text",
              text: `Loaded pipeline "${result.name}" (${result.agentCount} agents) from ${filePath}\n\n${summary}`,
            }],
            details: snapshot(),
          };
        }
        ...
        return {
          content: [{ type: "text", text: `Error: preset or file "${params.name}" not found.\nUse action 'list' to see available presets, or provide a valid file path.` }],
          isError: true,
        };
      } catch (err) {
        return {
          content: [{ type: "text", text: `Error loading pipeline: ${err instanceof Error ? err.message : String(err)}` }],
          isError: true,
        };
      }
    },
    ...
    renderCall: (args, theme) =>
      new Text(
        ...
        theme.fg("accent", args.action === "list" ? "list" : (args.name ?? "")),
        0, 0,
      ),
    renderResult: (result, _opts, theme) => {
      if (result.isError) return new Text(theme.fg("error", "✗ Load failed"), 0, 0);
  ...
  pi.registerTool({
    ...
    async execute(_id, params) {
      try {
        ...
        // Basic validation
        if (!spec.kind) {
          return { content: [{ type: "text", text: "Error: spec must have a 'kind' field (step, sequential, pool, parallel)" }], isError: true };
        }

        // Validate all agent references in the spec exist
        const unknownAgents = collectAgentRefs(spec).filter((name) => !agents[name]);
        if (unknownAgents.length > 0) {
          const available = Object.keys(agents).join(", ");
          return {
            content: [{ type: "text", text: `Error: unknown agent(s): ${unknownAgents.join(", ")}.\nAvailable agents: ${available}` }],
            isError: true,
          };
        }
        ...
        return {
          content: [{ type: "text", text: `Captain pipeline "${params.name}" defined:\n${summary}` }],
          details: snapshot(),
        };
      } catch (err) {
        return {
          content: [{ type: "text", text: `Error parsing pipeline spec: ${err instanceof Error ? err.message : String(err)}` }],
          isError: true,
        };
      }
    },
    ...
    renderCall: (args, theme) =>
      new Text(theme.fg("toolTitle", theme.bold("captain_define ")) + theme.fg("accent", args.name), 0, 0),
    renderResult: (result, _opts, theme) => {
      if (result.isError) return new Text(theme.fg("error", "✗ Invalid spec"), 0, 0);
      const d = result.details as CaptainDetails | undefined;
      const count = d ? Object.keys(d.pipelines).length : 0;
      return new Text(theme.fg("success", `✓ ${count} pipeline(s) defined`), 0, 0);
    },
  });
  ...
  pi.registerTool({
    name: "captain_run",
    label: "Captain Run",
    description: "Execute a defined captain pipeline. Runs steps according to composition rules (sequential/parallel/pool), manages git worktrees for isolation, chains $INPUT/$ORIGINAL through prompts, evaluates gates, handles failures. Returns final output.",
    parameters: Type.Object({
      name: Type.String({ description: "Pipeline name to run" }),
      input: Type.String({ description: "User's original request (becomes $ORIGINAL and initial $INPUT)" }),
    }),
    ...
    async execute(_id, params, signal, onUpdate, ctx) {
      ...
      if (!pipeline) {
        return {
          content: [{ type: "text", text: `Error: pipeline "${params.name}" not found. Define it first with captain_define.` }],
          isError: true,
        };
      }

      // Pre-flight: validate all agent references before running
      const unknownAgents = collectAgentRefs(pipeline.spec).filter((name) => !agents[name]);
      if (unknownAgents.length > 0) {
        const available = Object.keys(agents).join(", ");
        return {
          content: [{ type: "text", text: `Error: pipeline references unknown agent(s): ${unknownAgents.join(", ")}.\nAvailable agents: ${available}` }],
          isError: true,
        };
      }
      ...
      if (!apiKey) {
        return {
          content: [{ type: "text", text: "Error: no API key available for the current model" }],
          isError: true,
        };
      }
      ...
      const ectx: ExecutorContext = {
        ...
        onStepEnd: (result) => {
          ...
          updateWidget(ctx, state);
          onUpdate?.({
            content: [{ type: "text", text: `${statusIcon(result.status)} ${result.label}: ${result.status} (${(result.elapsed / 1000).toFixed(1)}s)` }],
          });
        },
      };
      ...
      try {
        ...
        const elapsed = ((state.endTime - (state.startTime ?? state.endTime)) / 1000).toFixed(1);
        const summary = [
        ...
        return {
          content: [{ type: "text", text: `Pipeline "${params.name}" failed: ${err instanceof Error ? err.message : String(err)}` }],
          details: snapshot(state),
          isError: true,
        };
      }
    ...
    renderCall: (args, theme) =>
      new Text(
        ...
        theme.fg("dim", ` "${(args.input as string).slice(0, 60)}${(args.input as string).length > 60 ? "…" : ""}"`),
        0, 0,
      ),
    renderResult: (result, { isPartial }, theme) => {
      if (isPartial) return new Text(theme.fg("warning", "⏳ Running pipeline..."), 0, 0);
      if (result.isError) return new Text(theme.fg("error", "✗ Pipeline failed"), 0, 0);
      const d = result.details as CaptainDetails | undefined;
      if (!d?.lastRun) return new Text(theme.fg("success", "✓ Done"), 0, 0);
      const s = d.lastRun.state;
      const elapsed = s.endTime && s.startTime ? ((s.endTime - s.startTime) / 1000).toFixed(1) : "?";
      return new Text(
        theme.fg("success", `✓ ${s.results.length} steps in ${elapsed}s`) +
        theme.fg("dim", ` (${s.results.filter(r => r.status === "passed").length}✓ ${s.results.filter(r => r.status === "failed").length}✗)`),
        0, 0,
      );
    },
  });
  ...
  pi.registerTool({
    ...
    description: "Check status of a running or completed captain pipeline. Shows step-by-step results, gates, and errors.",
    parameters: Type.Object({
      name: Type.String({ description: "Pipeline name" }),
    }),
    ...
    async execute(_id, params) {
      if (!runningState || runningState.name !== params.name) {
        const pipeline = pipelines[params.name];
        if (!pipeline) {
          return { content: [{ type: "text", text: `Pipeline "${params.name}" not found.` }], isError: true };
        }
        return { content: [{ type: "text", text: `Pipeline "${params.name}" defined but has not been run yet.` }], details: snapshot() };
      }
    ...
    renderCall: (args, theme) =>
      new Text(theme.fg("toolTitle", theme.bold("captain_status ")) + theme.fg("muted", args.name), 0, 0),
  });
  ...
  pi.registerTool({
    ...
    async execute(_id, _params, _signal, _onUpdate, ctx) {
      ...
      // Also show available but unloaded built-in presets
      const builtinNames = Object.keys(builtinPresetMap);
      const unloaded = builtinNames.filter(n => !pipelines[n]);
      ...
      // Check for project-local JSON pipelines too
      const cwd = ctx?.cwd ?? process.cwd();
      const projectDir = join(cwd, ".pi", "pipelines");
      if (existsSync(projectDir)) {
        const jsonFiles = readdirSync(projectDir).filter(f => f.endsWith(".json"));
        const unloadedJson = jsonFiles.map(f => basename(f, ".json")).filter(n => !pipelines[n]);
        if (unloadedJson.length > 0) {
      ...
      if (lines.length === 0) {
        return { content: [{ type: "text", text: "No pipelines defined or available. Use captain_define to create one." }], details: snapshot() };
      }

      const loaded = names.length;
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      goal: Type.String({ description: "What you want the pipeline to accomplish" }),
      dryRun: Type.Optional(Type.Boolean({ description: "If true, show the generated spec without registering it" })),
    }),
    ...
    async execute(_id, params, signal, onUpdate, ctx) {
      ...
      if (agentCount === 0) {
        return {
          content: [{ type: "text", text: "No agents available. Define agents with captain_agent or add .md files to ~/.pi/agent/agents/" }],
          isError: true,
        };
      }

      onUpdate?.({
        content: [{ type: "text", text: `🧠 Generating pipeline for: "${params.goal}" (${agentCount} agents available)...` }],
      });
      ...
      try {
        ...
        return {
          content: [{ type: "text", text: `Pipeline generation failed: ${err instanceof Error ? err.message : String(err)}` }],
          isError: true,
        };
      }
    },
    ...
    renderCall: (args, theme) =>
      new Text(
        ...
        theme.fg("accent", `"${(args.goal as string).slice(0, 50)}${(args.goal as string).length > 50 ? "…" : ""}"`),
        0, 0,
      ),
    renderResult: (result, _opts, theme) => {
      if (result.isError) return new Text(theme.fg("error", "✗ Generation failed"), 0, 0);
  ...
  pi.registerCommand("captain", {
    description: "Show pipeline details (/captain <name>) or list all (/captain)",
    ...
    handler: async (args, ctx) => {
      ...
      if (!name) {
        const names = Object.keys(pipelines);
        ctx.ui.notify(
          names.length ? `Pipelines: ${names.join(", ")}` : "No pipelines defined.",
          "info",
        );
        return;
      }
      ...
      ctx.ui.notify(`Pipeline "${name}":\n${describeRunnable(p.spec, 0)}`, "info");
    },
  });

  pi.registerCommand("captain-agents", {
    description: "List all available agents (from .md files and runtime definitions)",
  ...
  pi.registerCommand("captain-load", {
    description: "Load a pipeline preset (/captain-load <name>). No args to list available presets.",
    ...
    handler: async (args, ctx) => {
      ...
      if (!name) {
        ...
        const presets = discoverPresets(ctx.cwd);
        if (presets.length === 0) {
          ctx.ui.notify("No pipeline presets found. Place .json files in .pi/pipelines/", "info");
          return;
        }
        const lines = presets.map(p => `• ${p.name} (${p.source})`).join("\n");
      ...
      try {
        ...
        if (builtinPresetMap[name]) {
          ...
          if (!existsSync(projectFile)) {
            ctx.ui.notify(`Preset "${name}" not found. Run /captain-load to see available presets.`, "error");
            return;
          }
        ...
        ctx.ui.notify(`Failed to load: ${err instanceof Error ? err.message : String(err)}`, "error");
      }
    },
  });

  pi.registerCommand("captain-generate", {
    description: "Generate a pipeline on-the-fly with LLM (/captain-generate <goal>)",
    handler: async (args, ctx) => {
      const goal = args?.trim();
      if (!goal) {
        ctx.ui.notify("Usage: /captain-generate <what you want the pipeline to do>", "error");
        return;
      }
  ...
  function updateWidget(ctx: ExtensionContext, state: PipelineState) {
    ctx.ui.setWidget("captain", (_tui, theme) => ({
      render(width: number): string[] {
        ...
        lines.push(theme.fg("accent", theme.bold(`  🚀 Captain: ${state.name}`)) + theme.fg("dim", ` (${elapsed}s)`));

        // Show step results
        for (const r of state.results.slice(-6)) { // Show last 6 steps
          const icon = statusIcon(r.status);
          const line = `  ${icon} ${r.label}: ${r.status} (${(r.elapsed / 1000).toFixed(1)}s)`;
          const color = r.status === "passed" ? "success" : r.status === "failed" ? "error" : "dim";
          lines.push(theme.fg(color, line));
        }

        if (state.results.length > 6) {
          lines.push(theme.fg("dim", `  … and ${state.results.length - 6} more steps`));
        }

        lines.push(theme.fg("accent", "─".repeat(width)));

        // Safety: truncate all lines to terminal width
...
function describeRunnable(r: Runnable, indent: number): string {
  ...
  switch (r.kind) {
    ...
    case "sequential": {
      const gateInfo = r.gate ? ` (gate: ${r.gate.type}, onFail: ${r.onFail?.action ?? "none"})` : "";
      return [
