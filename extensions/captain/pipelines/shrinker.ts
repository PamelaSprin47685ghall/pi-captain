...
import { none, retry, skip, assert, command } from "../gates/index.js";
...
const shrinkAndScore: Step = {
  ...
  agent: "shrinker",
  description: "Score complexity and re-split any unit above the Haiku-safe threshold",
  prompt:
...
const validate: Step = {
  ...
  label: "Validate",
  agent: "validator",
  description: "Flash dry-run: confirm each unit can be executed in a single pass with no ambiguity",
  prompt:
...
const resolveDependencies: Step = {
  ...
  agent: "resolver",
  description: "Build adjacency graph, detect cycles, topological sort into parallelizable execution layers",
  prompt:
...
const renderCanvas: Step = {
  ...
  description: "Convert the layered task tree into a backlog.canvas file for Obsidian",
  prompt: CANVAS_PROMPT,
  gate: command("bun /Users/pierre-mikel/.pi/agent/skills/json-canvas/scripts/validate-canvas.ts backlog.canvas"),
