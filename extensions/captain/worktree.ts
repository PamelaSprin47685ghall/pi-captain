...
interface ExecFn {
  (cmd: string, args: string[], opts?: { signal?: AbortSignal }): Promise<{
    stdout: string;
    stderr: string;
    code: number;
  }>;
...
export async function isGitRepo(exec: ExecFn, cwd: string): Promise<boolean> {
  try {
    const { code } = await exec("git", ["-C", cwd, "rev-parse", "--is-inside-work-tree"]);
    return code === 0;
  } catch {
    return false;
  }
...
export async function createWorktree(
  ...
  const sanitized = `${pipelineName}-${branchLabel}-${index}`.replace(/[^a-zA-Z0-9_-]/g, "_");
  ...
  try {
    ...
    await exec("git", ["-C", cwd, "worktree", "remove", worktreePath, "--force"], { signal }).catch(() => {});
    await exec("git", ["-C", cwd, "branch", "-D", branchName], { signal }).catch(() => {});

    // Create worktree with a fresh branch based on HEAD
    await exec("git", ["-C", cwd, "worktree", "add", worktreePath, "-b", branchName], { signal });
...
export async function removeWorktree(
  ...
  try {
    await exec("git", ["-C", cwd, "worktree", "remove", worktreePath, "--force"], { signal });
  } catch {
