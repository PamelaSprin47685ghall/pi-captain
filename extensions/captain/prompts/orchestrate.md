...
2. **Design the pipeline** — choose composition patterns:
   ...
   - Parallel with repeated steps for diverse approaches to the same problem
...
- Use parallel with repeated steps (×3 with vote/rank) for creative or uncertain tasks: `{ kind: "parallel", steps: [step, step, step], merge: vote }`
