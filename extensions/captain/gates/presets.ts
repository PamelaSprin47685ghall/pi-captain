...
export function outputIncludes(needle: string): Gate {
  return { type: "assert", fn: `output.includes('${needle.replace(/'/g, "\\'")}')` };
}

/** Assert the output contains a string (case-insensitive) */
export function outputIncludesCI(needle: string): Gate {
  return { type: "assert", fn: `output.toLowerCase().includes('${needle.toLowerCase().replace(/'/g, "\\'")}')` };
}
...
export function regexExcludes(pattern: string, flags?: string): Gate {
  ...
  return command(`echo "${escapeForBash("$OUTPUT")}" | grep -qvP '${pattern}' || exit 1`);
}
...
export function httpStatus(url: string, status: number, method: string = "GET"): Gate {
...
export const fullCI = commandAll("bun test", "bunx tsc --noEmit", "bun run lint");

// ── Docker / Service Gates ────────────────────────────────────────────────

/** Check a Docker container is running by name */
export function dockerRunning(containerName: string): Gate {
  return command(`docker ps --format '{{.Names}}' | grep -q '^${containerName}$'`);
}
...
export const noConflicts = command("! grep -rn '<<<<<<< ' --include='*.ts' --include='*.js' --include='*.tsx' --include='*.jsx' .");
 ...
 */
export function llm(prompt: string, opts?: { model?: string; threshold?: number }): Gate {
  return { type: "llm", prompt, model: opts?.model, threshold: opts?.threshold };
}
