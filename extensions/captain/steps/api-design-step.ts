...
export const apiDesignStep: Step = {
  ...
  label: "API Design",
  agent: "architect",
  description: "Design the API contract including endpoints, schemas, and error handling",
  prompt:
  ...
  gate: assert("output.includes('GET') || output.includes('POST') || output.includes('endpoint') || output.includes('route')"),
  onFail: retry(2),
  transform: { kind: "full" },
};
