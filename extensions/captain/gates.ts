...
interface GateContext {
  exec: (cmd: string, args: string[], opts?: { signal?: AbortSignal }) => Promise<{ stdout: string; stderr: string; code: number }>;
...
export async function evaluateGate(
  ...
  switch (gate.type) {
    ...
    case "command": {
      // Run shell command — exit 0 = pass. Output is available via $OUTPUT env var.
      try {
        ...
        return code === 0
          ? { passed: true, reason: `Command passed: ${stdout.trim().slice(0, 200)}` }
          : { passed: false, reason: `Command failed (exit ${code}): ${(stderr || stdout).trim().slice(0, 200)}` };
      } catch (err) {
        return { passed: false, reason: `Command error: ${err instanceof Error ? err.message : String(err)}` };
      }
    }

    case "user": {
      // Human approval — skip if no UI available
    ...
    case "file": {
      ...
      try {
        const { code } = await gctx.exec("test", ["-f", gate.value], { signal: gctx.signal });
        return code === 0
          ? { passed: true, reason: `File exists: ${gate.value}` }
          : { passed: false, reason: `File not found: ${gate.value}` };
      } catch {
        return { passed: false, reason: `File check error: ${gate.value}` };
      }
    }

    case "assert": {
      // Evaluate a restricted assertion against the output string.
      // Only allows safe string operations — no arbitrary code execution.
      try {
        ...
        return { passed: false, reason: `Assertion error: ${err instanceof Error ? err.message : String(err)}` };
      }
    }
    ...
    case "regex": {
      // Test output against a regex pattern with optional flags
      try {
        const re = new RegExp(gate.pattern, gate.flags ?? "");
        const matched = re.test(output);
        return matched
          ? { passed: true, reason: `Regex matched: /${gate.pattern}/${gate.flags ?? ""}` }
          : { passed: false, reason: `Regex did not match: /${gate.pattern}/${gate.flags ?? ""}` };
      } catch (err) {
    ...
    case "json": {
      // Validate output is parseable JSON, optionally checking shape
      try {
        const parsed = JSON.parse(output);

        // If a schema descriptor is provided, validate required top-level keys
        if (gate.schema) {
          const requiredKeys = gate.schema.split(",").map((k) => k.trim());
          const missing = requiredKeys.filter((k) => !(k in parsed));
          if (missing.length > 0) {
            return { passed: false, reason: `JSON missing keys: ${missing.join(", ")}` };
          }
        }

        return { passed: true, reason: "Valid JSON" + (gate.schema ? ` with keys: ${gate.schema}` : "") };
      } catch {
    ...
    case "http": {
      // HTTP health check — fetch a URL and check status code
      ...
      try {
        ...
        const { stdout, code } = await gctx.exec("bash", ["-c", curlCmd], { signal: gctx.signal });
        const statusCode = parseInt(stdout.trim(), 10);
        if (statusCode === expected) {
          return { passed: true, reason: `HTTP ${method} ${gate.url} → ${statusCode}` };
        }
        return { passed: false, reason: `HTTP ${method} ${gate.url} → ${statusCode} (expected ${expected})` };
      } catch (err) {
        return { passed: false, reason: `HTTP check failed: ${err instanceof Error ? err.message : String(err)}` };
      }
    }

    case "multi": {
      // Combine multiple gates with AND ("all") or OR ("any") logic
      ...
      if (gate.mode === "all") {
        ...
        if (failed.length === 0) {
          return { passed: true, reason: `All ${results.length} gates passed` };
        }
        return { passed: false, reason: `${failed.length}/${results.length} gates failed: ${failed.map((f) => f.reason).join("; ")}` };
      } else {
        // At least one must pass
        const passed = results.filter((r) => r.passed);
        if (passed.length > 0) {
          return { passed: true, reason: `${passed.length}/${results.length} gates passed (any mode)` };
        }
        return { passed: false, reason: `All ${results.length} gates failed: ${results.map((r) => r.reason).join("; ")}` };
      }
    }
    ...
    case "dir": {
      ...
      try {
        const { code } = await gctx.exec("test", ["-d", gate.value], { signal: gctx.signal });
        return code === 0
          ? { passed: true, reason: `Directory exists: ${gate.value}` }
          : { passed: false, reason: `Directory not found: ${gate.value}` };
      } catch {
        return { passed: false, reason: `Directory check error: ${gate.value}` };
      }
    }

    case "env": {
      // Check environment variable is set, optionally matching a value
      try {
        const cmd = gate.value
          ? `test "$${gate.name}" = "${gate.value}"`   // exact match
          : `test -n "$${gate.name}"`;                  // just check non-empty
        const { code } = await gctx.exec("bash", ["-c", cmd], { signal: gctx.signal });
        return code === 0
          ? { passed: true, reason: `Env ${gate.name} is set` + (gate.value ? ` = ${gate.value}` : "") }
    ...
    case "timeout": {
      // Wrap any gate evaluation with a timeout — fails if gate takes too long
      const inner = evaluateGate(gate.gate, output, gctx);
      const timer = new Promise<GateResult>((resolve) =>
        setTimeout(() => resolve({ passed: false, reason: `Gate timed out after ${gate.ms}ms` }), gate.ms),
      );
      return Promise.race([inner, timer]);
    }

    case "llm": {
      // LLM-evaluated gate — ask an LLM to judge whether output meets criteria
      if (!gctx.model || !gctx.apiKey) {
        return { passed: false, reason: "LLM gate requires model and apiKey in context" };
      }

      // Resolve optional model override (e.g. "flash" for cheaper evaluation)
      const model = gate.model
        ? resolveGateModel(gate.model, gctx)
        : gctx.model;
      ...
      try {
        const response = await complete(model, {
          ...
          apiKey: gctx.apiKey,
          maxTokens: 512,      // Keep evaluation responses short
          signal: gctx.signal,
        });
...
function evaluateAssert(expr: string, output: string): boolean {
  ...
  const negInclude = trimmed.match(/^!output(?:\.toLowerCase\(\))?\.includes\((['"])(.*?)\1\)$/);
  if (negInclude) {
    const needle = negInclude[2] ?? "";
    const haystack = trimmed.includes(".toLowerCase()") ? output.toLowerCase() : output;
    return !haystack.includes(needle);
  }

  // output.includes(...) or output.toLowerCase().includes(...)
  const include = trimmed.match(/^output(?:\.toLowerCase\(\))?\.includes\((['"])(.*?)\1\)$/);
  if (include) {
  ...
  // output.length <op> N
  const lengthCmp = trimmed.match(/^output\.length\s*(>=|<=|>|<|===|!==)\s*(\d+)$/);
  if (lengthCmp) {
  ...
  throw new Error(`Unsupported assert expression: "${trimmed}". Supported: output.includes('...'), output.length > N, and ||/&& combinations.`);
}
 ...
 */
function resolveGateModel(modelName: string, gctx: GateContext): any {
