...
import type {
  ...
  StepResult, Agent, Transform, OnFail, Gate,
} from "./types.js";
...
import type { AgentContext, AgentLoopConfig, AgentEvent, AgentMessage, AgentTool } from "@mariozechner/pi-agent-core";
...
export interface ExecutorContext {
  ...
  model: any;                          // current model from ctx.model
  modelRegistry: any;                  // ctx.modelRegistry for resolving agent models
  apiKey: string;
  cwd: string;
  hasUI: boolean;
...
export async function executeRunnable(
  ...
  switch (runnable.kind) {
    ...
    default:
      return { output: `Unknown runnable kind: ${(runnable as any).kind}`, results: [] };
  }
}
...
async function executeStep(
  ...
  try {
    ...
    if (!agent) {
      ...
      throw new Error(`Agent "${step.agent}" not found. Available agents: ${available}`);
    }
    ...
    const output = await runAgentLoop(step, agent, prompt, tools, model, apiKey, ectx);
    ...
    if (!gateResult.passed) {
      ...
      const failResult = await handleFailure(step, input, original, output, gateResult, ectx, 0);
...
async function runAgentLoop(
  ...
  if (ectx.signal) {
    if (ectx.signal.aborted) {
      ...
      ectx.signal.addEventListener("abort", () => loopAbort.abort(), { once: true });
    }
  }
  ...
  const stream = agentLoop([userMessage], agentContext, config, loopAbort.signal);
  ...
  for await (const event of stream) {
    ...
    switch (event.type) {
      case "turn_start":
        turnCount++;
        // Safety: abort if too many turns (prevents runaway loops)
      ...
      case "message_end": {
        // Extract text from each assistant message
      ...
      // Tool execution events — relay progress to the widget
      case "tool_execution_start":
        ectx.onStepStart?.(`${step.label} → ${event.toolName}`);
        break;
    }
  }

  // The final output is the last assistant message's text content
...
async function gateCheck(
  ...
  switch (onFail.action) {
    case "retry": {
      ...
      return gateCheck(
        retried.output, retried.results,
        gate, onFail, scopeLabel, rerunFn,
        ectx, retryCount + 1,
      );
    }
    ...
    case "fallback": {
      ...
      return { output: fallback.output, results: [...results, gateStepResult, ...fallback.results] };
    }
...
async function executeSequential(
  ...
  for (const step of seq.steps) {
    ...
    const { output, results } = await executeRunnable(step, currentInput, original, ectx);
...
async function executePool(
  ...
  try {
    ...
    const promises = Array.from({ length: pool.count }, async (_, i) => {
      ...
      const wt = await createWorktree(ectx.exec, ectx.cwd, ectx.pipelineName, label, i, ectx.signal);
      ...
      return executeRunnable(pool.step, `${input}\n[Branch ${i + 1} of ${pool.count}]`, original, branchCtx);
    });
    ...
    for (const wt of worktrees) {
      await removeWorktree(ectx.exec, ectx.cwd, wt.path, wt.branch, ectx.signal);
    }
  }
}
...
function interpolatePrompt(template: string, input: string, original: string): string {
  return template
...
function resolveModel(agent: Agent | undefined, ectx: ExecutorContext): any {
      ...
      try {
        const response = await complete(
          ectx.model,
          {
            messages: [{
              role: "user",
              content: [{ type: "text", text: `Summarize concisely in 2-3 sentences:\n\n${output.slice(0, 4000)}` }],
              timestamp: Date.now(),
            }],
          },
...
async function handleFailure(
  ...
  ectx: ExecutorContext,
  retryCount: number,
): Promise<{ status: "passed" | "failed" | "skipped"; output: string; error?: string }> {
  ...
  switch (onFail.action) {
    case "retry": {
      const max = onFail.max ?? 3;
      if (retryCount >= max) {
        return { status: "failed", output: lastOutput, error: `Gate failed after ${max} retries: ${gateResult.reason}` };
      }
      ...
      const { output, results } = await executeStep(retryStep, input, original, ectx);
      const lastResult = results[results.length - 1];
      if (lastResult?.status === "passed") {
        return { status: "passed", output };
      }
      // Recursive retry
      return handleFailure(step, input, original, output, lastResult?.gateResult ?? gateResult, ectx, retryCount + 1);
    }
    ...
    case "retryWithDelay": {
      ...
      const max = onFail.max ?? 3;
      if (retryCount >= max) {
        return { status: "failed", output: lastOutput, error: `Gate failed after ${max} retries (with ${onFail.delayMs}ms delay): ${gateResult.reason}` };
      }
    ...
    case "skip":
      return { status: "skipped", output: "", error: `Skipped: ${gateResult.reason}` };

    case "warn":
      // Pass through the output despite the gate failure — non-blocking
      return { status: "passed", output: lastOutput, error: `⚠️ Warning (gate failed but continued): ${gateResult.reason}` };
...
function getLabel(r: Runnable): string {
  switch (r.kind) {
    case "step": return r.label;
    case "sequential": return `seq-${r.steps[0] ? getLabel(r.steps[0]) : "empty"}`;
    case "pool": return `pool-${getLabel(r.step)}`;
